...
DroneController::DroneController(const DataStructs::DroneInputData& input, const float& simStep)
{
	...
	armamentController = { inputData.AmmoType, inputData.AttackSpeed, inputData.Position.Z };
...
{
	targetsOperationalData[targetIndex].AngleToTarget = droneCalculator.CalculateAngleBetweenDroneAndTarget(operationalData.transform.Position, operationalData.transform.Direction, targetPosition);
...
void DroneController::CalculateFirePointsToTargets()
{
	...
	for (size_t i = 0; i < targets.size(); ++i)
	{
		...
		{
			DataStructs::Position2D maneuverPoint = droneCalculator.CalculateManeuverPosition(minAttackDistance, distanceToTarget, dronePosition, targetPosition);
			...
			DataStructs::Position2D firePoint = droneCalculator.CalculateFirePosition(armFallDistance, minAttackDistance, maneuverPoint, targetPosition);
		...
		{
			DataStructs::Position2D firePoint = droneCalculator.CalculateFirePosition(armFallDistance, minAttackDistance, dronePosition, targetPosition);
			targetsOperationalData[i].FirePoint.X = firePoint.X;
			targetsOperationalData[i].FirePoint.Y = firePoint.Y;
			targetsOperationalData[i].FirePoint.Z = 0;// point initialized
		}
...
{
	if (operationalData.State != DataStructs::TURNING &&
		operationalData.State != DataStructs::STOPPED)
	{
	...
	float angleToTarget = droneCalculator.CalculateAngleBetweenDroneAndTarget(
		calculatedState.Position,
		calculatedState.Direction,
		targetPosition);
...
void DroneController::RecalculateDronePosition(float simStepTime)
{
	if (calculatedState.Velocity > 0)
	{
		...
		DataStructs::Position2D directionVector = droneCalculator.CalculateDirectionVector(calculatedState.Position.GetPosition2D(), calculatedState.Direction);
...
void DroneController::CheckIfDroneReachedFirePosition()
{
	...
	bool isReadyToFire = droneCalculator.IsDistanceBetweenPointSameAsNeeded(operationalData.transform.Position.GetPosition2D(), targetPosition, armamentController.GetFallDistance());
