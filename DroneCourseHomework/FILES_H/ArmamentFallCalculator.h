...
class ArmamentFallCalculator
{
	...
	private:float CalculateCoefficientB(const ArmamentDatabase::Data&, float droneAttackSpeed);
	private:float CalculateCoefficientC(const ArmamentDatabase::Data&, float droneZPosition);

	// Tartaglia/Cardano method
	private:float CalculateCardanoP(float a, float b);
	private:float CalculateCardanoQ(float a, float b, float c);
	private:float CalculateCardanoPHI(float p, float q);

	private:float CalculateFallingTime(float p, float fi, float a, float b);
};